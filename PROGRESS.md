...
Perfgate gem. Intentionally minimal — no auth, no rich text, no background
...
| Rails version | 7.1.3.4 | Matches the Perfgate gem's own test matrix (`activerecord ~> 7.1`) |
...
### Step 6 — Perfgate integration
- `perfgate.yml` uses the correct schema keys (`execution`, `metrics`, `comparison.practical_thresholds`, `policy`, `storage`)
- `spec/rails_helper.rb` requires `perfgate/rspec` and `database_cleaner/active_record`
- `spec/support/workload_fixtures.rb` — reset/reseed fixture for forked workload specs
- `spec/perfgate/workloads_spec.rb` — 5 workloads, all tagged with stable `perfgate: { id: "..." }`
- `perfgate run` verified end-to-end: all 5 workloads execute, stable timings

```
perfgate run: 5 workload(s) -> .perfgate/main/runs/<uuid>
...
### Step 7 — Git history
- `main` branch: healthy reference implementation
...
**`perfgate compare` not yet smoke-tested with regression branches.** The `perfgate run`
step works. The full `run → compare` pipeline was verified on the Perfgate gem's own
test suite and examples but not yet exercised with this app's regression branches.

**Dockerfile is a stale artifact.** `Dockerfile`
...
├── spec/
│   ├── perfgate/
│   │   └── workloads_spec.rb # 5 Perfgate workloads
...
1. **Smoke-test `perfgate compare` with a regression branch** — record a `main` run,
   switch branch, record candidate, compare; verify Perfgate reports the expected
   metric as FAIL/WARN.
